defmodule DecodeCoordWeb.NodeController do
  ...
  def register(
        conn,
        %{"node_id" => node_id_str, "node_url" => node_url, "shards" => node_shards_hex} = _params
      ) do
    ...
    case {
      ...
      # Invalid node URL:
      {_, parsed_url}
      when parsed_url.scheme not in ["http", "https"] or
             parsed_url.host in [nil, ""] ->
        conn
        |> put_status(:bad_request)
        |> put_view(DecodeCoordWeb.ErrorView)
        |> render("bad_request.json",
