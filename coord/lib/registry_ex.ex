defmodule RegistryEx do
  ...
  `RegistryEx.start_link/1`, it can be used to register and access named
  processes using the `{:via, RegistryEx, {registry, key}}` tuple:

      {:ok, _} = RegistryEx.start_link(keys: :unique, name: RegistryEx.ViaTest)
      name = {:via, RegistryEx, {RegistryEx.ViaTest, "agent"}}
  ...
  process:
      ...
      RegistryEx.lookup(RegistryEx.ViaTest, "agent")
      #=> [{self(), nil}]

  However, in some cases it may be desired to associate a value to the process
  using the alternate `{:via, RegistryEx, {registry, key, value}}` tuple:

      {:ok, _} = RegistryEx.start_link(keys: :unique, name: RegistryEx.ViaTest)
      name = {:via, RegistryEx, {RegistryEx.ViaTest, "agent", :hello}}
      {:ok, _} = Agent.start_link(fn -> 0 end, name: name)
      RegistryEx.lookup(RegistryEx.ViaTest, "agent")
      #=> [{self(), :hello}]

  To this point, we have been starting `RegistryEx` using `start_link/1`.
  Typically the registry is started as part of a supervision tree though:

      {RegistryEx, keys: :unique, name: RegistryEx.ViaTest}
  ...
  `RegistryEx` has a dispatch mechanism that allows developers to implement custom
  dispatch logic triggered from the caller. For example, let's say we have a
  duplicate registry started as so:

      {:ok, _} = RegistryEx.start_link(keys: :duplicate, name: RegistryEx.DispatcherTest)
  ...
  to it:
      ...
      {:ok, _} = RegistryEx.register(RegistryEx.DispatcherTest, "hello", {IO, :inspect})
  ...
  in the code above:
      ...
      RegistryEx.dispatch(RegistryEx.DispatcherTest, "hello", fn entries ->
        ...
        for {pid, {module, function}} <- entries do
          try do
            ...
            kind, reason ->
              ...
              Logger.error("RegistryEx.dispatch/3 failed with #{formatted}")
          end
        end
      end)
      # Prints #PID<...>
  ...
  concurrent environments:
      ...
      {:ok, _} =
        RegistryEx.start_link(
          keys: :duplicate,
          name: RegistryEx.PubSubTest,
          partitions: System.schedulers_online()
        )

      {:ok, _} = RegistryEx.register(RegistryEx.PubSubTest, "hello", [])

      RegistryEx.dispatch(RegistryEx.PubSubTest, "hello", fn entries ->
  ...
  def send({registry, key, _value}, msg) do
    RegistryEx.send({registry, key}, msg)
  end
  ...
  Manually it can be started as:
      ...
      RegistryEx.start_link(keys: :unique, name: MyApp.RegistryEx)

  In your supervisor tree, you would write:

      Supervisor.start_link([
        {RegistryEx, keys: :unique, name: MyApp.RegistryEx}
  ...
  or:
      ...
      Supervisor.start_link([
        {RegistryEx, keys: :unique, name: MyApp.RegistryEx, partitions: System.schedulers_online()}
      ], strategy: :one_for_one)
  ...
  def start_link(options) do
    ...
    # The @info format must be kept in sync with RegistryEx.Partition optimization.
    entries = [
      {@all_info, {keys, partitions, nil, nil, listeners}},
      {@key_info, {keys, partitions, nil}} | meta
    ]

    RegistryEx.Supervisor.start_link(keys, name, partitions, listeners, entries, compressed)
  end

  @doc false
  @deprecated "Use RegistryEx.start_link/1 instead"
  ...
  def child_spec(options) do
    %{
      id: Keyword.get(options, :name, RegistryEx),
      start: {RegistryEx, :start_link, [options]},
      type: :supervisor
    }
  end
  ...
  ## Examples
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.UpdateTest)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.UpdateTest, "hello", 1)
      iex> RegistryEx.lookup(RegistryEx.UpdateTest, "hello")
      [{self(), 1}]
      iex> RegistryEx.update_value(RegistryEx.UpdateTest, "hello", &(&1 + 1))
      {2, 1}
      iex> RegistryEx.lookup(RegistryEx.UpdateTest, "hello")
      [{self(), 2}]
  ...
  def update_value(registry, key, callback) when is_atom(registry) and is_function(callback, 1) do
    case key_info!(registry) do
      ...
      {kind, _, _} ->
        raise ArgumentError, "RegistryEx.update_value/3 is not supported for #{kind} registries"
    end
  end
  ...
  both from itself and other processes:
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.UniqueLookupTest)
      iex> RegistryEx.lookup(RegistryEx.UniqueLookupTest, "hello")
      []
      iex> {:ok, _} = RegistryEx.register(RegistryEx.UniqueLookupTest, "hello", :world)
      iex> RegistryEx.lookup(RegistryEx.UniqueLookupTest, "hello")
      [{self(), :world}]
      iex> Task.async(fn -> RegistryEx.lookup(RegistryEx.UniqueLookupTest, "hello") end) |> Task.await()
      [{self(), :world}]

  The same applies to duplicate registries:

      iex> RegistryEx.start_link(keys: :duplicate, name: RegistryEx.DuplicateLookupTest)
      iex> RegistryEx.lookup(RegistryEx.DuplicateLookupTest, "hello")
      []
      iex> {:ok, _} = RegistryEx.register(RegistryEx.DuplicateLookupTest, "hello", :world)
      iex> RegistryEx.lookup(RegistryEx.DuplicateLookupTest, "hello")
      [{self(), :world}]
      iex> {:ok, _} = RegistryEx.register(RegistryEx.DuplicateLookupTest, "hello", :another)
      iex> Enum.sort(RegistryEx.lookup(RegistryEx.DuplicateLookupTest, "hello"))
  ...
  key in a duplicate registry but with different values:
      ...
      iex> RegistryEx.start_link(keys: :duplicate, name: RegistryEx.MatchTest)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.MatchTest, "hello", {1, :atom, 1})
      iex> {:ok, _} = RegistryEx.register(RegistryEx.MatchTest, "hello", {2, :atom, 2})
      iex> RegistryEx.match(RegistryEx.MatchTest, "hello", {1, :_, :_})
      [{self(), {1, :atom, 1}}]
      iex> RegistryEx.match(RegistryEx.MatchTest, "hello", {2, :_, :_})
      [{self(), {2, :atom, 2}}]
      iex> RegistryEx.match(RegistryEx.MatchTest, "hello", {:_, :atom, :_}) |> Enum.sort()
      [{self(), {1, :atom, 1}}, {self(), {2, :atom, 2}}]
      iex> RegistryEx.match(RegistryEx.MatchTest, "hello", {:"$1", :_, :"$1"}) |> Enum.sort()
      [{self(), {1, :atom, 1}}, {self(), {2, :atom, 2}}]
      iex> guards = [{:>, :"$1", 1}]
      iex> RegistryEx.match(RegistryEx.MatchTest, "hello", {:_, :_, :"$1"}, guards)
      [{self(), {2, :atom, 2}}]
      iex> guards = [{:is_atom, :"$1"}]
      iex> RegistryEx.match(RegistryEx.MatchTest, "hello", {:_, :"$1", :_}, guards) |> Enum.sort()
      [{self(), {1, :atom, 1}}, {self(), {2, :atom, 2}}]
  ...
  Registering under a unique registry does not allow multiple entries:
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.UniqueKeysTest)
      iex> RegistryEx.keys(RegistryEx.UniqueKeysTest, self())
      []
      iex> {:ok, _} = RegistryEx.register(RegistryEx.UniqueKeysTest, "hello", :world)
      iex> RegistryEx.register(RegistryEx.UniqueKeysTest, "hello", :later) # registry is :unique
  ...
  Such is possible for duplicate registries though:
      ...
      iex> RegistryEx.start_link(keys: :duplicate, name: RegistryEx.DuplicateKeysTest)
      iex> RegistryEx.keys(RegistryEx.DuplicateKeysTest, self())
      []
      iex> {:ok, _} = RegistryEx.register(RegistryEx.DuplicateKeysTest, "hello", :world)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.DuplicateKeysTest, "hello", :world)
      iex> RegistryEx.keys(RegistryEx.DuplicateKeysTest, self())
      ["hello", "hello"]
  ...
  both from itself and other processes:
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.UniqueLookupTest)
      iex> RegistryEx.values(RegistryEx.UniqueLookupTest, "hello", self())
      []
      iex> {:ok, _} = RegistryEx.register(RegistryEx.UniqueLookupTest, "hello", :world)
      iex> RegistryEx.values(RegistryEx.UniqueLookupTest, "hello", self())
      [:world]
      iex> Task.async(fn -> RegistryEx.values(RegistryEx.UniqueLookupTest, "hello", self()) end) |> Task.await()
      []
      iex> parent = self()
      iex> Task.async(fn -> RegistryEx.values(RegistryEx.UniqueLookupTest, "hello", parent) end) |> Task.await()
      [:world]

  The same applies to duplicate registries:

      iex> RegistryEx.start_link(keys: :duplicate, name: RegistryEx.DuplicateLookupTest)
      iex> RegistryEx.values(RegistryEx.DuplicateLookupTest, "hello", self())
      []
      iex> {:ok, _} = RegistryEx.register(RegistryEx.DuplicateLookupTest, "hello", :world)
      iex> RegistryEx.values(RegistryEx.DuplicateLookupTest, "hello", self())
      [:world]
      iex> {:ok, _} = RegistryEx.register(RegistryEx.DuplicateLookupTest, "hello", :another)
      iex> Enum.sort(RegistryEx.values(RegistryEx.DuplicateLookupTest, "hello", self()))
      [:another, :world]
  ...
  For unique registries:
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.UniqueUnregisterTest)
      iex> RegistryEx.register(RegistryEx.UniqueUnregisterTest, "hello", :world)
      iex> RegistryEx.keys(RegistryEx.UniqueUnregisterTest, self())
      ["hello"]
      iex> RegistryEx.unregister(RegistryEx.UniqueUnregisterTest, "hello")
      :ok
      iex> RegistryEx.keys(RegistryEx.UniqueUnregisterTest, self())
      []
  ...
  For duplicate registries:
      ...
      iex> RegistryEx.start_link(keys: :duplicate, name: RegistryEx.DuplicateUnregisterTest)
      iex> RegistryEx.register(RegistryEx.DuplicateUnregisterTest, "hello", :world)
      iex> RegistryEx.register(RegistryEx.DuplicateUnregisterTest, "hello", :world)
      iex> RegistryEx.keys(RegistryEx.DuplicateUnregisterTest, self())
      ["hello", "hello"]
      iex> RegistryEx.unregister(RegistryEx.DuplicateUnregisterTest, "hello")
      :ok
      iex> RegistryEx.keys(RegistryEx.DuplicateUnregisterTest, self())
      []
  ...
  the basis of whether or not it matches a particular value.
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.UniqueUnregisterMatchTest)
      iex> RegistryEx.register(RegistryEx.UniqueUnregisterMatchTest, "hello", :world)
      iex> RegistryEx.keys(RegistryEx.UniqueUnregisterMatchTest, self())
      ["hello"]
      iex> RegistryEx.unregister_match(RegistryEx.UniqueUnregisterMatchTest, "hello", :foo)
      :ok
      iex> RegistryEx.keys(RegistryEx.UniqueUnregisterMatchTest, self())
      ["hello"]
      iex> RegistryEx.unregister_match(RegistryEx.UniqueUnregisterMatchTest, "hello", :world)
      :ok
      iex> RegistryEx.keys(RegistryEx.UniqueUnregisterMatchTest, self())
      []
  ...
  For duplicate registries:
      ...
      iex> RegistryEx.start_link(keys: :duplicate, name: RegistryEx.DuplicateUnregisterMatchTest)
      iex> RegistryEx.register(RegistryEx.DuplicateUnregisterMatchTest, "hello", :world_a)
      iex> RegistryEx.register(RegistryEx.DuplicateUnregisterMatchTest, "hello", :world_b)
      iex> RegistryEx.register(RegistryEx.DuplicateUnregisterMatchTest, "hello", :world_c)
      iex> RegistryEx.keys(RegistryEx.DuplicateUnregisterMatchTest, self())
      ["hello", "hello", "hello"]
      iex> RegistryEx.unregister_match(RegistryEx.DuplicateUnregisterMatchTest, "hello", :world_a)
      :ok
      iex> RegistryEx.keys(RegistryEx.DuplicateUnregisterMatchTest, self())
      ["hello", "hello"]
      iex> RegistryEx.lookup(RegistryEx.DuplicateUnregisterMatchTest, "hello")
  ...
  Registering under a unique registry does not allow multiple entries:
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.UniqueRegisterTest)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.UniqueRegisterTest, "hello", :world)
      iex> RegistryEx.register(RegistryEx.UniqueRegisterTest, "hello", :later)
      {:error, {:already_registered, self()}}
      iex> RegistryEx.keys(RegistryEx.UniqueRegisterTest, self())
      ["hello"]

  Such is possible for duplicate registries though:

      iex> RegistryEx.start_link(keys: :duplicate, name: RegistryEx.DuplicateRegisterTest)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.DuplicateRegisterTest, "hello", :world)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.DuplicateRegisterTest, "hello", :world)
      iex> RegistryEx.keys(RegistryEx.DuplicateRegisterTest, self())
      ["hello", "hello"]
  ...
  ## Examples
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.MetaTest, meta: [custom_key: "custom_value"])
      iex> RegistryEx.meta(RegistryEx.MetaTest, :custom_key)
      {:ok, "custom_value"}
      iex> RegistryEx.meta(RegistryEx.MetaTest, :unknown_key)
      :error
  ...
  ## Examples
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.PutMetaTest)
      iex> RegistryEx.put_meta(RegistryEx.PutMetaTest, :custom_key, "custom_value")
      :ok
      iex> RegistryEx.meta(RegistryEx.PutMetaTest, :custom_key)
      {:ok, "custom_value"}
      iex> RegistryEx.put_meta(RegistryEx.PutMetaTest, {:tuple, :key}, "tuple_value")
      :ok
      iex> RegistryEx.meta(RegistryEx.PutMetaTest, {:tuple, :key})
      {:ok, "tuple_value"}
  ...
  ## Examples
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.DeleteMetaTest)
      iex> RegistryEx.put_meta(RegistryEx.DeleteMetaTest, :custom_key, "custom_value")
      :ok
      iex> RegistryEx.meta(RegistryEx.DeleteMetaTest, :custom_key)
      {:ok, "custom_value"}
      iex> RegistryEx.delete_meta(RegistryEx.DeleteMetaTest, :custom_key)
      :ok
      iex> RegistryEx.meta(RegistryEx.DeleteMetaTest, :custom_key)
      :error
  ...
  number of keys in the registry:
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.UniqueCountTest)
      iex> RegistryEx.count(RegistryEx.UniqueCountTest)
      0
      iex> {:ok, _} = RegistryEx.register(RegistryEx.UniqueCountTest, "hello", :world)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.UniqueCountTest, "world", :world)
      iex> RegistryEx.count(RegistryEx.UniqueCountTest)
      2

  The same applies to duplicate registries:

      iex> RegistryEx.start_link(keys: :duplicate, name: RegistryEx.DuplicateCountTest)
      iex> RegistryEx.count(RegistryEx.DuplicateCountTest)
      0
      iex> {:ok, _} = RegistryEx.register(RegistryEx.DuplicateCountTest, "hello", :world)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.DuplicateCountTest, "hello", :world)
      iex> RegistryEx.count(RegistryEx.DuplicateCountTest)
      2
  ...
  key in a duplicate registry but with different values:
      ...
      iex> RegistryEx.start_link(keys: :duplicate, name: RegistryEx.CountMatchTest)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.CountMatchTest, "hello", {1, :atom, 1})
      iex> {:ok, _} = RegistryEx.register(RegistryEx.CountMatchTest, "hello", {2, :atom, 2})
      iex> RegistryEx.count_match(RegistryEx.CountMatchTest, "hello", {1, :_, :_})
      1
      iex> RegistryEx.count_match(RegistryEx.CountMatchTest, "hello", {2, :_, :_})
      1
      iex> RegistryEx.count_match(RegistryEx.CountMatchTest, "hello", {:_, :atom, :_})
      2
      iex> RegistryEx.count_match(RegistryEx.CountMatchTest, "hello", {:"$1", :_, :"$1"})
      2
      iex> RegistryEx.count_match(RegistryEx.CountMatchTest, "hello", {:_, :_, :"$1"}, [{:>, :"$1", 1}])
      1
      iex> RegistryEx.count_match(RegistryEx.CountMatchTest, "hello", {:_, :"$1", :_}, [{:is_atom, :"$1"}])
      2
  ...
  This example shows how to get everything from the registry.
      ...
      iex> RegistryEx.start_link(keys: :unique, name: RegistryEx.SelectAllTest)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.SelectAllTest, "hello", :value)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.SelectAllTest, "world", :value)
      iex> RegistryEx.select(RegistryEx.SelectAllTest, [{{:"$1", :"$2", :"$3"}, [], [{{:"$1", :"$2", :"$3"}}]}])
  ...
  Get all keys in the registry.
      ...
      iex> {:ok, _} = RegistryEx.register(RegistryEx.SelectAllTest, "hello", :value)
      iex> {:ok, _} = RegistryEx.register(RegistryEx.SelectAllTest, "world", :value)
      iex> RegistryEx.select(RegistryEx.SelectAllTest, [{{:"$1", :_, :_}, [], [:"$1"]}])
      ["world", "hello"]
    ...
    spec =
      for part <- spec do
        case part do
          ...
          _ ->
            raise ArgumentError,
                  "invalid match specification in RegistryEx.select/2: #{inspect(spec)}"
        end
      end
...
defmodule RegistryEx.Supervisor do
  @moduledoc false
  use Supervisor
  ...
  def init({kind, registry, partitions, listeners, entries, compressed}) do
    ...
    children =
      for i <- 0..(partitions - 1) do
        key_partition = RegistryEx.Partition.key_name(registry, i)
        pid_partition = RegistryEx.Partition.pid_name(registry, i)
        arg = {kind, registry, i, partitions, key_partition, pid_partition, listeners, compressed}

        %{
          id: pid_partition,
          start: {RegistryEx.Partition, :start_link, [pid_partition, arg]}
        }
      end
...
defmodule RegistryEx.Partition do
  @moduledoc false
  ...
  def handle_info({:EXIT, pid, _reason}, {ets, registry}) do
    {_, _, _, _, listeners} = :ets.lookup_element(registry, @all_info, 2)

    :ets.take(ets, pid)
    |> Enum.map(fn {_pid, key, key_ets, _counter} ->
      key_ets =
        case key_ets do
      ...
      try do
        RegistryEx.__unregister__(key_ets, {key, {pid, :_}}, 1)
      catch
        :error, :badarg -> :badarg
      end
    ...
    |> then(fn pid_keys ->
      for listener <- listeners do
        Kernel.send(listener, {:exited, registry, pid, pid_keys, Process.whereis(registry)})
      end
    end)

    {:noreply, {ets, registry}}
  end
end
